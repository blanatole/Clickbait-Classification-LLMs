#!/usr/bin/env python3
"""
Run all clickbait classification experiments on CUDA GPUs
...
def main():
    ...
    print("🎯 CLICKBAIT CLASSIFICATION - CUDA GPU FULL EXPERIMENT SUITE")
    print("="*80)
    ...
    experiments = [
        {
            ...
            "command": "python fine-tuning/scripts/train_bert_family.py --model all",
            "description": "Train BERT-base, BERT-large",
            "expected_time_minutes": 120,  # ~2 hours
            "models": ["bert-base-uncased", "bert-large-uncased"]
        },
        {
            "name": "Mistral-7B-Instruct-v0.3",
            "command": "python fine-tuning/scripts/train_llm_lora.py --model mistral-7b-instruct",
            "description": "Train Mistral-7B-Instruct-v0.3 with QLoRA",
            "expected_time_minutes": 90,   # ~1.5 hours (fewer epochs)
            "models": ["mistral-7b-instruct"]
        },
        {
            "name": "Llama-3.1-8B-Instruct",
            "command": "python fine-tuning/scripts/train_llm_lora.py --model llama3-8b",
            "description": "Train Llama-3.1-8B-Instruct with QLoRA",
    ...
    if successful_experiments:
        ...
        try:
            subprocess.run("python shared/scripts/benchmark_results.py", shell=True, check=True)
