...
Optimized for CUDA GPUs with sufficient VRAM
...
# Model configurations optimized for high-end CUDA GPUs
MODEL_CONFIGS = {
    "bert-base-uncased": {
...
def train_model(model_key, config, output_base_dir="outputs"):
    ...
    # Setup paths
    output_dir = f"{output_base_dir}/{model_key}-cuda"
    ...
    training_args = TrainingArguments(
        ...
        report_to=None,
        logging_dir=f"{output_dir}/logs",  # Logs saved in output directory
...
def main():
    parser = argparse.ArgumentParser(description="Train BERT family models on CUDA GPUs")
    ...
    if torch.cuda.is_available():
        ...
        if gpu_memory < 16:
            print("⚠️  Warning: This script is optimized for GPUs with 16GB+ VRAM")
